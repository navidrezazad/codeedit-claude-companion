...
class CEOpenWith: FIFinderSync {
    ...
    func openInCodeEditAction(_ sender: AnyObject?) {
        guard let items = FIFinderSyncController.default().selectedItemURLs(),
              let defaults = UserDefaults.init(suiteName: "app.codeedit.CodeEditV2.shared") else {
            return
        }
        ...
        guard let codeEdit = NSWorkspace.shared.urlForApplication(
            withBundleIdentifier: "app.codeedit.CodeEditV2"
        ) else { return }
    ...
    override func menu(for menuKind: FIMenuKind) -> NSMenu {
        guard let defaults = UserDefaults.init(suiteName: "app.codeedit.CodeEditV2.shared") else {
